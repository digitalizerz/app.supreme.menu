...
<div  id="cartList" class="border-top">
    <div  v-for="item in items" class="items clearfix">
        <div style="display: inline-flex">
            <button style="z-index: 1" type="button" v-on:click="remove(item.id)"  :value="item.id" class="absolute close">
            ...
            <picture style="padding: 0px;width:76px">
                <img style="border-top-left-radius: 20px;" :src="item.attributes.image" :data-src="item.attributes.image"  class="productImage" alt="">
            </picture>
            <h6 class="title mb-1" style="margin-left: 10px;">@{{ item.name }}</h6>
            <div class="info" v-cloak>
                <div class="extras d-flex justify-content-between align-items-center">
                    <div class="qty d-flex justify-content-around align-items-center">
                        <button type="button" v-on:click="decQuantity(item.id)" :value="item.id" class="btn btn-icon btn-sm page-link btn-cart-radius">-</button>
                        <h5 class="product-item_quantity mx-3">@{{ item.quantity }}</h5>
                        <button type="button" v-on:click="incQuantity(item.id)" :value="item.id" class="btn btn-icon btn-sm page-link btn-cart-radius mr-0">+</button>
                    </div>
                    <div class="price">
